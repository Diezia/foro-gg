...
export abstract class EditRepositoryBase<
  TEntity,
  TId = number
> extends ReadonlyRepositoryBase<TEntity, TId> {
  constructor(
  ...
  insert(entity: TEntity): void {
    const insertCommand = new InsertDbCommand(this.connection, this.tableName, [
      entity,
    ]);
    this.batch.addCommand(
      insertCommand,
      (x: any) => ((entity as any)[this.idColumn] = x.insertId)
    );
  }

  replace(entity: TEntity): void {
    const replaceCommand = new ReplaceDbCommand(
      this.connection,
      this.tableName,
      [entity]
    );
  ...
  async update(entity: TEntity): Promise<TEntity> {
    await this.connection.connection.query(
      `UPDATE \`${this.tableName}\` SET ? WHERE \`${this.idColumn}\`=?`,
      [entity, (entity as any)[this.idColumn]]
    );
    return entity;
  }
