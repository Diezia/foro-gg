...
export class PostRepository extends EditRepositoryBase<Post, number> {
  ...
  async getOrderedPostsByDateByGameId(where: string, args: any): Promise<Post[]> {
    const [rows] = await this.connection.connection.execute(`SELECT * FROM \`${this.tableName}\` WHERE ${where} ORDER BY created_at DESC`, args);
